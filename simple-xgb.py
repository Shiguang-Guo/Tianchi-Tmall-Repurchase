...
# 1 for <18; 2 for [18,24]; 3 for [25,29]; 4 for [30,34]; 5 for [35,39]; 6 for [40,49]; 7 and 8 for >= 50; 0 and NULL
# for unknown
...
print(matrix)
matrix.to_csv(os.path.join(feature_path, 'data_1.csv'), index=False)
...
model = MLPClassifier(solver='lbfgs', activation='relu', alpha=0.1, random_state=0, hidden_layer_sizes=[10, 10]).fit(
    train_X, train_y)
